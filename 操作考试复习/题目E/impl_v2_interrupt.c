...
volatile u8 g_flow_speed = 0;    // 0=1s,1=0.5s,2=0.25s
volatile u8 g_led_shifted = 0;
volatile u8 g_scroll_speed = 0;  // 0=1s,1=0.5s
...
void ButtonISR(void) {
  ...
  if (pressed & BTN_C) {
    ...
    g_mode = MODE_BIN; g_led_val = 0xAA; g_led_shifted = 0;
  } else if (pressed & BTN_R) {
    g_mode = MODE_INV; g_led_val = 0x55; g_led_shifted = 0;
  } else if (pressed & BTN_U) {
    g_mode = MODE_SCROLL; g_scroll_speed = !g_scroll_speed;
    g_scroll_offset = 0; g_led_shifted = 0;
  }
...
int main() {
  ...
  Xil_Out32(TIMER_BASE + XTC_TLR_OFFSET, 24999998);  // 0.25s
